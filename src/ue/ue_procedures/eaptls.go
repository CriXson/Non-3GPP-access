...
//var LayerTypeEAP = gopacket.RegisterLayerType(55, gopacket.LayerTypeMetadata{Name: "EAP", Decoder: gopacket.DecodeFunc(decodeEAP)})
...
//func (e *EAP) LayerType() gopacket.LayerType { return nil }

// DecodeFromBytes decodes the given bytes into this layer.
func (e *EAPTLS) DecodeFromBytes(data []byte, df gopacket.DecodeFeedback) error {
	if len(data) < 4 {
		df.SetTruncated()
...
func (e *EAPTLS) SerializeTo(b gopacket.SerializeBuffer, opts gopacket.SerializeOptions) error {
	if opts.FixLengths {
		e.Length = uint16(len(e.TypeData))
	}
	size := len(e.TypeData) + 6
	/*
		if size > 6 {
			size++
		}
	*/
	...
	if size > 4 {
		bytes[4] = byte(e.Type)
		bytes[5] = byte(e.Flags)

	}
	if size > 6 {
		if e.Flags&(1<<7) > 0 {
			binary.BigEndian.PutUint32(bytes[6:], e.TLSMessageLength)
			copy(bytes[10:], e.TypeData)
		} else {
			copy(bytes[6:], e.TypeData)
		}
...
func (e *EAPTLS) NextLayerType() gopacket.LayerType {
